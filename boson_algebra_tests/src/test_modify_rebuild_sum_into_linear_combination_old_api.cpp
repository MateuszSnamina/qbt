...
TEST(ModifyRebuildSumIntoLinearCombinationOldApi, OnBosonPrimitiveOperator) {
    const auto expression = 'd'_cr;
    ASSERT_EQ(expression.str(), "♯d");
    //std::cout << expression.str() << std::endl;
    const auto transformed_expression_optional = modify_rebuild_sum_into_linear_combination(expression);
    ASSERT_FALSE(transformed_expression_optional);
}

TEST(ModifyRebuildSumIntoLinearCombinationOldApi, OnEmptyProduct) {
...
TEST(ModifyRebuildSumIntoLinearCombinationOldApi, OnEmptySum) {
...
TEST(ModifyRebuildSumIntoLinearCombinationOldApi, OnProductOfBosonPrimitiveOperators) {
...
TEST(ModifyRebuildSumIntoLinearCombinationOldApi, OnSumOfBosonPrimitiveOperators) {
    const auto expression = ('a'_cr + 'b'_an);
    //std::cout << expression.str() << std::endl;
    ASSERT_EQ(expression.str(), "❴♯a+♭b❵");
    const auto transformed_expression_optional = modify_rebuild_sum_into_linear_combination(expression);
...
TEST(ModifyRebuildSumIntoLinearCombinationOldApi, BigExample1) {
