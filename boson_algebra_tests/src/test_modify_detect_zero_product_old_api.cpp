...
TEST(ModifyDetectZeroProductOldApi, OnBosonPrimitiveOperator) {
...
TEST(ModifyDetectZeroProductOldApi, OnEmptyProduct) {
...
TEST(ModifyDetectZeroProductOldApi, OnProductOfBosonPrimitiveOperators) {
...
TEST(ModifyDetectZeroProductOldApi, OnFactorExpression) {
...
TEST(ModifyDetectZeroProductOldApi, OnNestedFactorExpression) {
...
TEST(ModifyDetectZeroProductOldApi, OnZeroProduct1) {
...
TEST(ModifyDetectZeroProductOldApi, OnZeroProduct2) {
...
TEST(ModifyDetectZeroProductOldApi, OnSumWithZero) {
    const auto expression = (ba::SumExpression::make_zero() + 'b'_an);
    //std::cout << expression.str() << std::endl;
    ASSERT_EQ(expression.str(), "❴𝟘+♭b❵");
    const auto transformed_expression_optional = modify_detect_zero_product(expression);
