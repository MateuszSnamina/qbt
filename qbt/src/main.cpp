...
ba::ExpressionHandler do_simplification(ba::ExpressionHandler&& expression) {
    ...
    std::cout << "Flaten: " << expression.str() << std::endl;

    expression.safe_dfs_transform(ba::transform_sum_to_linear_combination);
...
int main() {
    ...
    auto expression1 = do_simplification(std::move(expression));
    std::cout << "OUTPUT:" << expression1.str() << std::endl;
}